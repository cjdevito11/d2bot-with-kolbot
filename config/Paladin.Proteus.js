...
function LoadConfig() {
	...
	Scripts.ClearAnyArea = false; // Clear any area. Uses Config.ClearType to determine which type of monsters to kill.
		Config.ClearAnyArea.AreaList = [129,130,131]; // List of area ids to clear. See sdk/areas.txt
	...
	Scripts.Baal = true;
		...
		Config.Baal.BaalMessage = "Thats It!";
...
*/
	...
	Config.Leader = "Trangouls"; // Leader's ingame character name. Leave blank to try auto-detection (works in AutoBaal, Wakka, MFHelper)
	...
	Scripts.Wakka = false; // Walking chaos leecher with auto leader assignment, stays at safe distance from the leader
		//Config.Wakka.SkipIfBaal = true; // End script if there are party members in a Baal run.
	...
	Scripts.BaalHelper = false;
		...
		Config.BaalHelper.KillBaal = false; // Kill Baal. If set to false, you must configure Config.QuitList or the bot will wait indefinitely.
	...
	Scripts.BaalAssistant = false; // Used to leech or help in baal runs.
		...
		Config.BaalAssistant.KillBaal = false; // Set to true to kill baal, if you set to false you MUST configure Config.QuitList or Config.BaalAssistant.NextGameMessage or the bot will wait indefinitely.
	...
	// Town settings
	Config.HP = 30; // Go to a healer if under designated percent of life.
	...
	Config.CainID.MinGold = 1000; // Minimum gold (stash + character) to have in order to use Cain.
	...
	Config.ScanShrines = [15,12,13,14,11,10,9,8,7,6,5,4,3,2];
	...
	Config.AutoBuild.Template = "AuradinBuild";	//	The name of the build associated with an existing
